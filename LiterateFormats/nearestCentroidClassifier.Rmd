---
title: "Nearest Centroid Classification"
author: "William Bell"
date: '2018-09-13'
...
```{r centroidvis, echo = FALSE}
set.seed(10)

SimData1 <- data.frame(X = runif(4), Y = rnorm(4))

Centroid1 <- apply(SimData1, 2, mean)

plot(rbind(SimData1, Centroid1), 
     col = c(rep("blue", 4), "red"), 
     main = "Centroid of a Simulated Dataset", 
     pch = 19)
...
```{r twocluster, echo = FALSE}

set.seed(111)

SimData2 <- data.frame(X = rnorm(4, mean = 2, sd = 0.25), Y = rnorm(4, mean = 2))

Centroid2 <- apply(SimData2, 2, mean)

Everything <- data.frame(X = rep(NA, 10), Y = rep(NA, 10))

Everything[1:4, ] <- SimData1

Everything[5, ] <- Centroid1

Everything[6:9, ] <- SimData2

Everything[10, ] <- Centroid2

plot(Everything, col = c(rep("blue", 4), "red", rep("dark green", 4), "purple"), main = "Centroid of Simulated Datasets", pch = 19)
...
```{r, echo = FALSE}
plot(simulatedData, main = "Simulated Dataset")
```
...
print(c("Proportion Correct" = proportionCorrect))
